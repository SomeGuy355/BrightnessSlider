...
def watcher_thread():
    ...
    try:
        ...
        for session in sessions:
            if session.Process:
                if 'brightness' in session.Process.name().lower():  # check process name
                    brightness_session = session.SimpleAudioVolume
                elif 'contrast' in session.Process.name().lower():
                    contrast_session = session.SimpleAudioVolume

        old_brightness = int(round((brightness_session.GetMasterVolume() * 100)/10)*10)
        old_contrast = int(round((contrast_session.GetMasterVolume() * 100)/10)*10)
        ...
        while True:
            brightness = int(round(brightness_session.GetMasterVolume() * 100/10)*10)       # round it to 10
            contrast   = int(round(contrast_session.GetMasterVolume()   * 100/10)*10)
            if brightness_session.GetMute() != brightness_mute:
                subprocess.run([CM_EXE,'x'])
                sleep(5)
